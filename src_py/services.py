...
from src_py.database.models import QueueItem
...
class ConversationService:
    ...
    def create_conversation(self, contact_id: int, title: str | None = None, metadata: dict[str, Any] | None = None, queue_id: int | None = None):
        return self.conversation_repo.create(contact_id=contact_id, title=title, metadata=metadata, queue_id=queue_id)

    def get_or_create_conversation(self, contact_id: int, title: str | None = None, queue_id: int | None = None):
        conversation = self.conversation_repo.get_active_by_contact(contact_id)
        if conversation:
            if queue_id and not conversation.queue_id:
                conversation.queue_id = queue_id
                self.db.commit()
                self.db.refresh(conversation)
            return conversation
        return self.create_conversation(contact_id=contact_id, title=title, queue_id=queue_id)
...
class QueueService:
    ...
    def add_to_queue(self, queue_id: int, conversation_id: int) -> Optional[int]:
        ...
        existing = (
            ...
            .filter(QueueItem.queue_id == queue_id, QueueItem.conversation_id == conversation_id, QueueItem.status.in_(["waiting", "processing"]))
            .first()
        )
        if existing:
            return existing.id
