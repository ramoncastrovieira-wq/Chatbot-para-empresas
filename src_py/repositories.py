...
class ConversationRepository(BaseRepository):
    def create(self, contact_id: int, title: str | None = None, metadata: dict[str, Any] | None = None) -> Conversation:
        conversation = Conversation(contact_id=contact_id, title=title, metadata_=metadata or {}, status="open")
    ...
    def get_active_by_contact(self, contact_id: int) -> Optional[Conversation]:
        return self.db.query(Conversation).filter(
            and_(Conversation.contact_id == contact_id, Conversation.status == "open")
...
class QueueRepository(BaseRepository):
    ...
    def list_all(self, skip: int = 0, limit: int = 100) -> list[Queue]:
        return self.db.query(Queue).order_by(desc(Queue.priority), Queue.name).offset(skip).limit(limit).all()
